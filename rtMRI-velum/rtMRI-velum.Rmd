---
title: "Planting the seed for sound change: evidence from real-time MRI of German velum kinematics"
output: 
  html_document: 
    highlight: tango
    number_sections: yes
...
The following distributions were used as weakly informative priors (on the log-odds scale): for the intercept of duration (corresponding to ND and overall mean speech rate), a normal distribution with mean = 0 and standard deviation = 3 ($Normal(0, 3)$); for the effect of voicing (when NT), $Normal(0, 1)$.
...
For the correlation between random effects, an $LKJ(2)$ distribution, as recommended by [...].
